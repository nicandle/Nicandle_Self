# -----------------------------------------------------------------------------
# Batch Excel to CSV Converter with Settings Persistence (v4.1 - Corrected)
# Author: AI聊天机器人
# Current Time: Fri Jul 18 13:09:19 CST 2025
# -----------------------------------------------------------------------------
...
class ExcelBatchConverterApp:
    """
    一个批量将指定目录及子目录下的Excel文件转换为CSV的应用程序，
    并能在目标目录中保持原始的文件夹结构。
    新增功能：关闭时保存源/目标目录，启动时自动加载。
    """
    def __init__(self, root):
        self.root = root
        self.root.title("批量 Excel 转 CSV 转换器 v4.1 (带记忆功能)")
        ...
        ttk.Label(main_frame, text="源目录 (如: .../config):").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ...
        ttk.Label(main_frame, text="目标目录 (如: .../config):").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        ...
        list_frame.grid(row=2, column=0, columnspan=3, padx=5, pady=10, sticky="nsew")
        ...
        action_frame.grid(row=3, column=0, columnspan=3, pady=5, sticky="ew")
    ...
    def scan_files(self):
        ...
        for relative_path, full_path in found_files:
            ...
            cb = ttk.Checkbutton(self.checkbox_frame, text=relative_path, variable=var)
            cb.pack(anchor="w", padx=10, pady=2)
            self.file_checkboxes.append((var, full_path))
        self.status_label.config(text=f"扫描完成！发现 {len(self.file_checkboxes)} 个文件。")
